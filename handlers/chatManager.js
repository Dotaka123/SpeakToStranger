...
class ChatManager {
    constructor() {
        ...
        // 🛡️ Anti-spam : { userId -> [timestamps des messages] }
        this.spamTracker = new Map();
        // LIMITE : 5 messages max par fenêtre de 3 secondes
        this.SPAM_LIMIT = 5;
        this.SPAM_WINDOW_MS = 3000;

        // ⏱️ Historique des temps d'attente pour l'estimation (30 derniers matchs)
        this.matchTimings = [];
        this.MAX_TIMING_SAMPLES = 30;
    }
    ...
    recordMatchTiming(waitMs) {
        this.matchTimings.push(waitMs);
        if (this.matchTimings.length > this.MAX_TIMING_SAMPLES) {
            this.matchTimings.shift();
        }
    }

    getEstimatedWaitTime() {
        if (this.matchTimings.length < 3) return null; // Pas assez de données
        const avg = this.matchTimings.reduce((a, b) => a + b, 0) / this.matchTimings.length;
        return Math.round(avg / 1000); // En secondes
    }

    formatWaitEstimate(seconds) {
    ...
    checkSpam(userId) {
        ...
        const recent = timestamps.filter(t => now - t < this.SPAM_WINDOW_MS);
        recent.push(now);
        this.spamTracker.set(userId, recent);
        
        return recent.length > this.SPAM_LIMIT; // true = spam détecté
    }
    ...
    try {
        ...
        try {
            ...
            if (matchFound && matchedPartner) {
                // Enregistrer le temps d'attente du partenaire pour l'estimation
                const waitMs = Date.now() - matchedPartner.joinedAt.getTime();
                this.recordMatchTiming(waitMs);
                ...
                // 🔔 Notifier les utilisateurs qui attendaient une notif de disponibilité
                await this.notifyWaitingUsers();
                ...
                const estimatedSec = this.getEstimatedWaitTime();
                const estimatedStr = this.formatWaitEstimate(estimatedSec);
                const onlineCount = this.activeChats.size / 2 + this.waitingQueue.length;

                // Message avec position + estimation
                ...
                if (estimatedStr) {
                    waitMessage += `⏱️ Temps estimé : ${estimatedStr}\n`;
                }
                waitMessage += `🌐 En ligne : ${onlineCount} personne(s)\n\n`;
                ...
                console.log(`📋 Ajouté à la queue: ${pseudo} (Position ${queuePosition}${estimatedStr ? `, estimé ${estimatedStr}` : ''})`);
            }
    ...
    // Appelé après chaque match : notifie les users qui avaient demandé une alerte
    async notifyWaitingUsers() {
        try {
            const usersToNotify = await User.find({ notifyWhenAvailable: true });
            for (const user of usersToNotify) {
                // Ne pas notifier quelqu'un déjà en queue ou en chat
                if (this.isInQueue(user.facebookId) || this.isInChat(user.facebookId)) continue;
                ...
                const message = {
                    text: `🔔 Une personne vient de se connecter !\n\n` +
                          `C'est le bon moment pour chercher un partenaire.\n\n` +
                          `Voulez-vous lancer une recherche maintenant ?`,
                    quick_replies: [
                        { content_type: "text", title: "🔍 Oui, chercher !", payload: "QUICK_CHERCHER" },
                        { content_type: "text", title: "❌ Non merci", payload: "QUICK_NOTIFY_CANCEL" }
                    ]
                };
                await this.fb.sendQuickReplies(user.facebookId, message);
                console.log(`🔔 Notification envoyée à ${user.pseudo}`);
            }
        } catch (error) {
    ...
    // Proposer la notification quand la queue est vide (appelé par addToQueue)
    async proposeNotification(userId) {
        try {
            const message = {
                text: `😕 Personne disponible pour le moment.\n\n` +
                      `Voulez-vous être notifié dès qu'une personne arrive ?`,
                quick_replies: [
                    { content_type: "text", title: "🔔 Oui, notifie-moi", payload: "QUICK_NOTIFY_YES" },
                    { content_type: "text", title: "❌ Non merci", payload: "QUICK_NOTIFY_NO" }
                ]
            };
            await this.fb.sendQuickReplies(userId, message);
        } catch (error) {
            console.error('Erreur proposeNotification:', error);
        }
    }
    ...
    async createChatSafe(user1, user2) {
        try {
            ...
            // 💬 QUESTION BRISE-GLACE (envoyée 1.5s après pour ne pas noyer le message de connexion)
            const icebreaker = this.getIcebreaker(theme1, theme2);
            if (icebreaker) {
                setTimeout(async () => {
                    try {
                        const icebreakerMsg = `💡 Pour briser la glace :\n"${icebreaker}"`;
                        await Promise.all([
                            this.fb.sendTextMessage(user1.userId, icebreakerMsg),
                            this.fb.sendTextMessage(user2.userId, icebreakerMsg)
                        ]);
                    } catch (e) { /* silencieux */ }
                }, 1500);
            }
    ...
    // ========================================
    // 💬 BRISE-GLACES PAR THÈME
    // ========================================

    getIcebreaker(theme1, theme2) {
        const ICEBREAKERS = {
            '⚽ Sport': [
                "Tu suis quel sport en ce moment ?",
                "Tu as une équipe favorite ?",
                "Tu pratiques un sport ou tu préfères regarder ?"
            ],
            '🎵 Musique': [
                "Quel genre de musique tu écoutes en ce moment ?",
                "Tu joues d'un instrument ?",
                "Quel artiste tu ne te lasses jamais d'écouter ?"
            ],
            '🎮 Gaming': [
            ...
            '📚 Culture': [
                ...
                "Dernier film ou série qui t'a marqué ?",
                "Tu préfères les livres, films ou podcasts ?"
            ],
            '🌍 Voyage': [
                "Quel est l'endroit que tu as préféré visiter ?",
                "Tu as un pays de rêve où tu veux aller ?",
                "Tu préfères la mer, la montagne ou la ville ?"
            ],
            '💡 Tech': [
                "Tu suis les actus tech ou tu codes ?",
                "Quel gadget tu ne pourrais pas vivre sans ?",
                "Tu penses quoi de l'IA en ce moment ?"
            ],
            '🎨 Art': [
                "Tu créés quelque chose en ce moment ?",
                "Quel style d'art tu apprécies le plus ?",
                "Tu préfères les musées, concerts ou expositions ?"
            ]
        };

        const GENERIC = [
            "Tu fais quoi aujourd'hui ?",
            "Tu es de quelle région du monde ?",
            "Qu'est-ce qui t'a donné envie de discuter ce soir ?",
            "Tu as un talent caché ?"
        ];

        // Chercher une question commune si même thème
        if (theme1 && theme1 === theme2 && ICEBREAKERS[theme1]) {
            const list = ICEBREAKERS[theme1];
            return list[Math.floor(Math.random() * list.length)];
        }

        // Sinon chercher dans le thème de l'un des deux
        const activeTheme = theme1 || theme2;
        if (activeTheme && ICEBREAKERS[activeTheme]) {
            const list = ICEBREAKERS[activeTheme];
            return list[Math.floor(Math.random() * list.length)];
        }
    ...
    async removeFromQueue(userId, offerNotification = true) {
        await this.acquireMatchingLock();
        try {
            const wasInQueue = this.waitingQueue.some(u => u.userId === userId);
            ...
            // 🔔 Proposer une notification si la queue est vide après son départ
            if (wasInQueue && offerNotification && this.waitingQueue.length === 0) {
                setTimeout(() => this.proposeNotification(userId), 500);
            }
        } finally {
            this.releaseMatchingLock();
        }
    ...
    async endChat(userId, reason = 'user_request') {
        ...
        try {
            ...
            // 🛡️ Nettoyer le tracker anti-spam
            this.resetSpam(userId);
            this.resetSpam(chat.partnerId);
    ...
    async relayMessage(senderId, message) {
        ...
        // 🛡️ VÉRIFICATION ANTI-SPAM (texte uniquement)
        if (message.text) {
            const isSpam = this.checkSpam(senderId);
            if (isSpam) {
                try {
                    const user = await User.findOne({ facebookId: senderId });
                    const warnings = (user?.spamWarnings || 0) + 1;
                    ...
                    if (warnings >= 2) {
                        ...
                        await this.fb.sendTextMessage(senderId,
                            ...
                            "━━━━━━━━━━━━━━━━━━\n\n" +
                            "Trop de messages en trop peu de temps.\n" +
                            "Conversation terminée.\n\n" +
                            "Tapez /chercher pour recommencer."
                        );
                        await this.fb.sendTextMessage(chat.partnerId,
                            "⚠️ Votre partenaire a été déconnecté pour spam.\n\n" +
                            "Tapez /chercher pour trouver un nouveau partenaire."
                        );
                        await this.endChat(senderId, 'spam');
                    } else {
                        // 1er avertissement
                        await this.fb.sendTextMessage(senderId,
                            "⚠️ ATTENTION - ENVOI TROP RAPIDE\n" +
                            "━━━━━━━━━━━━━━━━━━\n\n" +
                            "Vous envoyez des messages trop vite.\n" +
                            "Ralentissez ou vous serez déconnecté."
                        );
                    }
                    return false;
                } catch (spamError) {
                    console.error('Erreur traitement spam:', spamError);
                }
                return false;
            }
        }
