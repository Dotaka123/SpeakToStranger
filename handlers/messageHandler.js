...
class MessageHandler {
        ...
        try {
            switch(payload) {
                ...
                case 'QUICK_NOTIFY_CANCEL':
                    await this.fb.sendTextMessage(senderId,
                        "👍 D'accord, pas de notification.\n\n" +
                        "Tapez /chercher quand vous voulez réessayer !"
                    );
                    break;
    ...
    // 🔔 Activer la notification de disponibilité
    async enableNotification(senderId) {
        try {
            ...
            await this.fb.sendTextMessage(senderId,
                ...
                "Vous recevrez un message dès qu'une personne sera disponible.\n\n" +
                "Tapez /stop à tout moment pour annuler."
            );
        } catch (error) {
            console.error('Erreur enableNotification:', error);
            await this.fb.sendTextMessage(senderId, "❌ Erreur lors de l'activation.");
        }
    }
